# 📡 Monitoring and Scaling

> Part of [Rails Deployment and DevOps](https://stanza.dev/courses/rails-deployment-devops)

Monitor production applications and scale to handle growth.

## Lessons

1. [Application Monitoring](./1-application-monitoring.md)
2. [Logging Observability](./2-logging-observability.md)
3. [Horizontal Scaling](./3-horizontal-scaling.md)
4. [Alerting Incident Response](./4-incident-response.md)

## Practice Challenges

This section includes **3 interactive challenges** available on Stanza:

- 🧩 Multiple Choice: Production Logging
- 🔗 Matching: Monitoring Scaling Challenge
- 🧩 Challenge: Incident Response Steps

→ [Practice in your IDE](https://stanza.dev/courses/rails-deployment-devops)