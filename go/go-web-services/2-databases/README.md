# 🗄️ Database Interactions

> Part of [Go for Web & Microservices](https://stanza.dev/courses/go-web-services)

Using database/sql securely.

## Lessons

1. [database/sql Basics](./1-sql-package.md)
2. [Queries & Prepared Statements](./2-sql-queries.md)
3. [Transactions](./3-sql-transactions.md)
4. [SQL Security & Best Practices](./4-sql-security.md)

## Practice Challenges

This section includes **4 interactive challenges** available on Stanza:

- 🧩 Multiple Choice: SQL Injection Prevention
- 🧩 Multiple Choice: DB Connection Pool
- 🧩 Challenge: Transaction Lifecycle
- 🔗 Matching: Database Functions

→ [Practice in your IDE](https://stanza.dev/courses/go-web-services)