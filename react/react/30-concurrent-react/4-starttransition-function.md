...
1. **Using for async operations**: `startTransition` doesn't wait for promises. The callback must be synchronous.
...
3. **Wrapping async function**: This doesn't work as expected:

```tsx
// WRONG - async functions return promises
startTransition(async () => {
  const data = await fetchData(); // This doesn't defer!
  setData(data);
});
```
...
- For async operations, set state inside the callback after awaiting outside
